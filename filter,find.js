const data = [
  ...
  const sl_btn = document.querySelector('#find');

  const find_email = function(){
    const name_value = document.querySelector('#user-name').value;
    const number_value = document.querySelector('#phone-number').value;
    const user = data.find(ele => {
        return ele.userName === name_value && ele.phoneNumber === number_value; 
    }) 
    const message = user ? `${user.userName}님의 이메일은 ${user.email} 입니다.` : '이메일을 찾을 수 없습니다. 입력 정보를 다시 확인해 주세요.';
    alert(message);
  }

sl_btn.addEventListener('click', find_email)
